...
public class CrazyGenerics {
     ...
     */
//    todo:!!! start hard
    // generic type need to be specify in the lower class and after that can be used in the upper class
    ...
    static class CollectionUtil {
         ...
         */
        //todo: not so clear task. It is difficult to understand what exactly targetEntity
        public static <T extends BaseEntity> boolean hasDuplicates(Collection<T> entities, T targetEntity) {
            return entities.stream().filter(t -> t.getUuid() == targetEntity.getUuid()).count() > 1;
        }
        ...
        public static <T> Optional<T> findMax(Iterable<T> elements,
